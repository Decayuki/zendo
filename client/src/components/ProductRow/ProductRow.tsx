...
// Fetch & map en fonction de l'endpoint fourni
// =============================================================
...
interface ProductRowProps {
  ...
  onProductClick: (product: any) => void; // fonction appelee si clic sur un produit
}
...
function ProductRow(props: ProductRowProps) {
      ...
      async function fetchProducts() {
        try {
          const response = await axios.get(
            "http://localhost:5001/api/products" + props.endpoint, //donc en vrai ça va donner "/api/products?sort=popular&limit=8"
          );
  ...
  return (
    <section className="product-row-section">
          ...
          {products.map(function (product) {
            return (
              <div key={product._id} className="product-row-item">
                <ProductCard product={product} onClick={props.onProductClick} />
              </div>
            );
          })}
        </div>
      ...
      {/* Aucun produit */}
      {!loading && !error && products.length === 0 && (
        <p className="product-row-empty">Aucun produit pour le moment</p>
      )}
    </section>
  );
}
